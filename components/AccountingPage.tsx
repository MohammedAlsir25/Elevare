...
const AccountingPage: React.FC = () => {
                                ...
                                <tr key={account.id} className="border-b border-gray-200 dark:border-gray-700 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    ...
                                    {permissions.canEditAccounting && (
                                        <td className="p-3 text-center">
                                            <button onClick={() => { setEditingAccount(account); setIsAccountModalOpen(true); }} className="text-gray-500 dark:text-gray-400 hover:text-brand-primary p-1"><EditIcon className="h-5 w-5" /></button>
                                            <button onClick={() => handleDeleteAccount(account.id)} className="text-gray-400 dark:text-gray-500 hover:text-accent-red p-1 ml-2"><DeleteIcon className="h-5 w-5" /></button>
                                        </td>
                                    )}
                                </tr>
                            ))}
                            ...
                            {sortedJournalEntries.map(entry => {
                                ...
                                return (
                                    <tr key={entry.id} className="border-b border-gray-200 dark:border-gray-700 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        ...
                                        {permissions.canEditAccounting && (
                                            <td className="p-3 text-center">
                                                <button onClick={() => { setEditingJournalEntry(entry); setIsJournalModalOpen(true); }} className="text-gray-500 dark:text-gray-400 hover:text-brand-primary p-1"><EditIcon className="h-5 w-5" /></button>
                                                <button onClick={() => handleDeleteJournal(entry.id)} className="text-gray-500 dark:text-gray-400 hover:text-accent-red p-1 ml-2"><DeleteIcon className="h-5 w-5" /></button>
                                            </td>
                                        )}
                                    </tr>
                                );
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="mt-4">
                {activeTab === 'accounts' ? renderAccountsContent() : renderJournalContent()}
            </div>
