                            ...
                            <tr key={product.id} className="border-b border-gray-200 dark:border-gray-700 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                {permissions.canEditInventory && (
                                    <td className="p-3 text-center">
                                        <button onClick={() => onEdit(product)} className="text-gray-500 dark:text-gray-400 hover:text-brand-primary p-1"><EditIcon className="h-5 w-5" /></button>
                                        <button onClick={() => onDelete(product.id)} className="text-gray-500 dark:text-gray-400 hover:text-accent-red p-1 ml-2"><DeleteIcon className="h-5 w-5" /></button>
                                    </td>
                                )}
                            </tr>
                        ))}
...
// --- Purchase Order Content Component ---
                ...
                <tbody>
                    {paginatedPOs.map(po => (
                         <tr key={po.id} className="border-b border-gray-200 dark:border-gray-700 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            ...
                            {permissions.canEditInventory && (
                                <td className="p-3 text-center">
                                    {po.status === PurchaseOrderStatus.ORDERED && <button onClick={() => onReceive(po.id)} className="text-accent-green hover:underline text-sm font-semibold">Receive</button>}
                                    <button onClick={() => onEdit(po)} className="text-gray-500 dark:text-gray-400 hover:text-brand-primary p-1 ml-2"><EditIcon className="h-5 w-5" /></button>
                                    <button onClick={() => onDelete(po.id)} className="text-gray-500 dark:text-gray-400 hover:text-accent-red p-1 ml-2"><DeleteIcon className="h-5 w-5" /></button>
                                </td>
                            )}
                         </tr>
                    ))}
